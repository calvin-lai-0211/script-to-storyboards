...
@router.post("/character/{character_id}/submit-task", response_model=SubmitTaskResponse)
async def submit_character_image_task(
    character_id: int,
    ...
    """
    Submit async image generation task for character portrait.
...
@router.get("/character/{character_id}/task-status/{task_id}", response_model=TaskStatusResponse)
async def get_character_task_status(
    character_id: int,
    task_id: str,
    response: Response,
    db: Database = Depends(get_db),
